...
| Field | Value |
|---|---|
| **Current Sprint** | Sprint 5.5 - Phase 3: OpenCode desktop as first-class command center - **COMPLETE (P1–P6 all SHIPPED)** |
| **Status** | **COMPLETE** - P1 (session bridge AGENTS.md, `f8c5389`) + P2 (session telemetry-on-close, `8e07d9d` + fix `16a43a2`) + P3 (dashboard -> desktop deep links, `ad488a8`) + P4 (desktop -> submit for review, `2d3b6d6`) + P5 (GUI/desktop action telemetry, `42ccc4e`) + **P6 (R3 parity milestone MET — 44/71 rows, `ae21380`)** SHIPPED; suite **1465 green**; **CI run `30893245070` 8/8 green (2026-08-04)** |
...
| **Commit** | Sprint 5.5: P6 `ae21380` / P5 `42ccc4e` / P4 `2d3b6d6` / P3 `ad488a8` / P1 `f8c5389` / P2 `8e07d9d` / P2 fix `16a43a2` / Sprint 5.4 closed `4ca321f` / T1 `294e107` / T2 `11fde80` / T3 `f690133` / T4 `9dba281` / T5 `b93a886` / Sprint 5.3 `de9c851` |
...
### Sprint 5.5 P1 Deliverables — Session bridge: constitution/state load on open (DONE)

- [x] **`AGENTS.md` (project root, committed)** — the OpenCode-native session bridge. OpenCode auto-loads `AGENTS.md` into every session's context (docs: "Set custom instructions for opencode ... included in the LLM's context"), so there is **no manual load step** and it works from the repo root in a fresh session. Content: the immutable Core Directives inline (Read State First / Pydantic v2 / no placeholders / strict typing / Update State Last) + explicit "read these files immediately at session start" instructions for `.ai-company/constitution/rules.md`, `.ai-company/state/current_sprint.yaml`, and `.ai/current-work.md`; quick command reference; frozen-CLI (ADR 0006), services-layer (ADR 0003), parity (R3), write-auth (ADR 0010), four-state status (R12) conventions. `.ai-company/` is gitignored (machine-local, single-machine deployment) — AGENTS.md instructs sessions to proceed without blocking if a file is missing.
...
### Sprint 5.5 P3 Deliverables — Deep link: dashboard → "continue in OpenCode" (DONE, `ad488a8`)

- [x] **Deep-link builder** - `services/deep_links.py` (Core Directive 2, Pydantic v2): `NewSessionDeepLink` model validates `directory` (1-1024) + `prompt` (1-2000, blank rejected) and emits `opencode://new-session?directory=<enc>&prompt=<enc>` (backslashes normalized, full percent-encoding via `urllib.parse.quote(safe='')`). Desktop session-id resume (`opencode://session/<id>`) is still an upstream feature request (anomalyco/opencode#6232), so links open a fresh session seeded with run/target/plan context. Per-surface prompt builders (`run_continue_prompt` / `target_continue_prompt` / `plan_continue_prompt`) + additive `enrich_run` / `enrich_target` / `enrich_plan` + `project_directory(config_dir)` (default `config` → its parent = repo root).
- [x] **Facade enrichment (ADR 0003 single surface)** - `generate_targets`, `generate_runs`, `generate_run`, `generate_start`, and `orchestration_history` each add a `deep_link` key to every record. No new endpoints; existing API + server-rendered views pick it up unchanged. Errors stay fail-open (empty `deep_link` never blocks a response).
- [x] **Dashboard** - `/generate` target rows and run-history Actions column ("Continue in OpenCode", `target="_blank" rel="noopener"`), plus per-plan links on `/runs`; CSS `.deep-link` (dashed accent underline + `.btn-sm` variant). Data flows: server-side via template context, run rows via `run.deep_link` in `app.js::loadGenerateRuns`.
...
### Sprint 5.5 P5 Deliverables — GUI/desktop action telemetry (honest D5 numerator) (DONE, `42ccc4e`)

- [x] **`telemetry/actions.py`** — fail-open JSONL at `runtime/action_telemetry.jsonl` (`_MAX_RECORDS = 5000` prune cap): `record_action(source, action, *, count, session_id, duration_seconds)` / `read_action_records(limit=200)` / `action_share_summary(window_days=30, target_pct=80.0)` — D5 share = (**gui** guarded dashboard writes + **desktop** explicit records + session commands/tool calls at the **newest checkpoint per session**, same dedupe as the Sessions panel) ÷ (**numerator + CLI invocations** from the signed-off baseline `runtime/cli_telemetry.jsonl`); returns counts/share_pct/at_target/by_action. Corrupt/missing logs contribute zero; every write fail-open.
- [x] **WriteGuard hook (source param)** - `guards.py::WriteGuard.audited(..., source="gui")` records a GUI operator action on every guarded mutation (the single choke point, ADR 0010); `source=None` opts out — `telemetry.session.persist` is plumbing, not an operator action. `review.submit` passes `source="desktop"`. Blocked (401/403) writes never record.
- [x] **Retention** - `action_telemetry` registered in `SOURCE_PATHS` + `DEFAULT_POLICIES` (180d, rollup false — per-action record, not a time series) + `config/runtime/telemetry.yaml`; `test_retention.py` policy-set assertions updated.
- [x] **API + facade** - `RuntimeFacade.action_telemetry_summary(window_days, target_pct)` (fail-open envelope) + `GET /api/telemetry/actions?window_days=&target_pct=` (`operational_endpoints.py`); contract-map key `telemetry_actions`; `/telemetry` view passes `actions_summary` and renders the **D5 card** (cols-4: share %, GUI+desktop total, CLI baseline, total actions + by-action line).
- [x] **Parity** - row added to `docs/dashboard/parity-matrix-v0.md` (**N/A** - read-only metric, no CLI counterpart; CLI surface frozen ADR 0006); guard test `tests/golden/test_parity_wave2b.py::test_parity_d5_actions_surface` asserts the summary is complete/bounded and `/api/telemetry/actions` is in the OpenAPI contract with window/target knobs.
- [x] **Test hermeticity** - the guarded-write suites (`test_write_endpoints.py`, `test_wave2b_endpoints.py`) gained an **autouse chdir-to-tmp_path fixture**: P5's file side-effect means non-isolated writes would pollute the live D5 metric from test runs (verified: without the fixture the full suite wrote test noise into `runtime/action_telemetry.jsonl`; with it, the repo tree stays clean).
...
11b. **Sprint 5.5 P3 SHIPPED (2026-08-03, `ad488a8`)** — deep link dashboard
    → "continue in OpenCode": every generate target, run, and orchestration
    plan carries an additive `deep_link` (`opencode://new-session?directory=
    &prompt=` from `services/deep_links.py`, Pydantic v2) surfaced as
    "Continue in OpenCode" links on `/generate` + `/runs`. No CLI counterpart
...
11d. **Sprint 5.5 P5 SHIPPED (2026-08-04, `42ccc4e`)** — GUI/desktop action
    telemetry = the honest D5 numerator. `telemetry/actions.py` computes the
    D5 share as (gui guarded dashboard writes + desktop records + OpenCode
    session commands/tool calls, newest-per-session) ÷ (numerator + CLI
    baseline `runtime/cli_telemetry.jsonl`), fail-open at
    `runtime/action_telemetry.jsonl`. The shared `WriteGuard.audited()`
    records every guarded mutation as a GUI action (single choke point,
    ADR 0010); `telemetry.session.persist` is excluded (`source=None`),
    `review.submit` is recorded under `desktop`. `GET /api/telemetry/actions`
    + `/telemetry` D5 card. Retention source `action_telemetry` (180d, no
    rollup). Parity row N/A + OpenAPI guard test; write-endpoint suites now
    chdir to tmp so tests never pollute the live metric. Suite **1428 green**;
    all gates clean; live-verified (review.submit → desktop 1, decision.create
    → gui 1, share 14.2% rendered).

11e. **Sprint 5.5 P6 SHIPPED (2026-08-04, `ae21380`)** — **R3 parity milestone
    MET: 44/71 command rows parity-tested (≥40 target)**, Phase 3 close-out
    satisfied. Wave 3 golden parity: `tests/golden/test_parity_wave3_read.py`
    (18 tests — `registry show` vision+board, `exec org-chart` Mermaid vs
    `/api/org-chart`, `memory list/get/search/show/stats/snapshots` (hermetic
    tmp-cwd engine; snapshots honest empty-case — default `MemorySnapshot` is
    in-memory per instance, `from_config` never wires the snapshot dir, a
    latent product gap to flag separately), `report generate summary/detailed/
    health`, `company board-report`, `orchestrate status/history`,
    `runtime health/metrics/diagnostics` via hermetic runtime); and
    `tests/golden/test_parity_wave3_writes.py` (19 tests — CLI `--help` knobs
    == OpenAPI body schemas for `memory save/update/snapshot/restore/archive/
    unarchive`, `runtime start/stop/restart/reload` (`--reason`), `orchestrate
    plan/start/resume/retry/rollback`, `report generate`, `graph export`,
    `bootstrap`, `build`). Known render notes asserted in-test: rich consumes
    `[ns] [type]` summary prefixes as markup (CLI line == API summary minus
    prefix), rich soft-wraps long mermaid lines (whitespace-normalized
    compare), Board block parsed separately from Executives in detailed
    report. Suite **1465 green**; ruff/mypy-strict/format/command-map/
    integrity clean; pre-commit green. **Sprint 5.5 P1–P6 all SHIPPED.**
