...
| OpenCode desktop deep link (P3) | (no CLI command — `opencode://` desktop URL scheme) | `deep_link` field on `/api/generate/targets`, `/api/generate/runs*`, `/api/orchestrate/history` | "Continue in OpenCode" links on Generate + Runs & History views | **SHIPPED** (Sprint 5.5 P3 — parity **N/A**: desktop-URL scheme, no CLI counterpart; guard `tests/golden/test_parity_wave2b.py::test_parity_deep_link_desktop_surface`) |
| OpenCode desktop submit-for-review (P4) | (no CLI command — desktop HTTP surface) | Guarded audited `POST /api/review/submit` → creates `review` decision + returns `review_link` (`/decisions?focus=<id>`) | `/decisions` desktop-source chip + artifact paths + `?focus=` highlight | **SHIPPED** (Sprint 5.5 P4 — parity **N/A**: desktop HTTP surface, no CLI counterpart, CLI surface frozen ADR 0006; guard `tests/golden/test_parity_wave2b.py::test_parity_review_submit_desktop_surface` — shares the WriteGuard 401/403) |
| GUI/desktop action telemetry (P5) | (no CLI command — D5 metric read) | `GET /api/telemetry/actions?window_days=&target_pct=` → D5 share (gui + desktop + session activity ÷ + CLI baseline) | `/telemetry` D5 north-star card | **SHIPPED** (Sprint 5.5 P5 — parity **N/A**: read-only metric, no CLI counterpart, CLI surface frozen ADR 0006; guard `tests/golden/test_parity_wave2b.py::test_parity_d5_actions_surface` — complete/bounded summary + OpenAPI contract) |
