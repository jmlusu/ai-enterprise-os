...
def validate_yaml_file(filepath: Path) -> ValidationReport:
    ...
    try:
        ...
        if data is None:
            report.add_warning(
                f"File returned no data: {filepath.name}", path=str(filepath)
            )
            return report
        ...
        report.add_error(
            f"Unexpected error parsing {filepath.name}: {e}", path=str(filepath)
        )
        return report
...
def validate_yaml_directory(registry_dir: Path) -> ValidationReport:
    ...
    if not registry_dir.is_dir():
        report.add_error(
            f"Registry directory not found: {registry_dir}", path=str(registry_dir)
        )
        return report
    ...
    for filename in REGISTRY_FILES:
        ...
        if not filepath.exists():
            report.add_warning(
                f"Optional registry file not found: {filename}", path=str(filepath)
            )
            continue
