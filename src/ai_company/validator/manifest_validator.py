...
def validate_manifest_file(manifest_path: Path) -> ValidationReport:
    ...
    if not manifest_path.exists():
        report.add_error(f"Manifest file not found: {manifest_path}", path=str(manifest_path))
        return report

    if manifest_path.stat().st_size == 0:
        report.add_error(f"Manifest file is empty: {manifest_path}", path=str(manifest_path))
        return report
    ...
    if not manifest.name:
        report.add_error("Manifest name is required", field="name", path=str(manifest_path))
    if manifest.version:
        ...
        report.add_warning("No version specified in manifest", field="version", path=str(manifest_path))

    if not manifest.description:
        report.add_warning("No description in manifest", field="description", path=str(manifest_path))

    if not manifest.company_name:
        report.add_warning("No company_name in manifest", field="company_name", path=str(manifest_path))
    ...
    if len(manifest.departments) > 0:
        ...
        if len(names) != len(set(names)):
            ...
            for d in set(dupes):
                report.add_error(f"Duplicate department name: {d}", field="departments", path=str(manifest_path))
        for dept in manifest.departments:
            if not dept.name:
                report.add_error("Department with empty name", field="departments", path=str(manifest_path))
