...
def generate(
    ...
    if report_type == "summary":
        ...
        print(f"  Vision: {reg.vision.name}")
        ...
        print(f"  Board members: {len(reg.board)}")
        print(f"  Workflows: {len(reg.workflows)}")
        if result.warnings:
            for warn in result.warnings:
                print(f"  [yellow]![/yellow] {warn}")
        ...
        if reg.vision.description:
            print(f"  Description: {reg.vision.description}")
        if reg.vision.company_name:
            print(f"  Company: {reg.vision.company_name}")
        print("\n  Departments:")
        ...
        if reg.workflows:
            ...
            for wf in reg.workflows:
                ...
                print(f"    - {wf.name or '(unnamed)'}: {wf.description or ''}{steps_desc}")
        ...
        print("[yellow]Available: summary, detailed, health[/yellow]")
        raise typer.Exit(1)
